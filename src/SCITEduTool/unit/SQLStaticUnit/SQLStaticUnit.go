...
func InitSQL(conf SQLConfig) {
	...
	if strings.Contains(conf.Username, "//") ||
		...
		StdOutUnit.Assert.String("", "用户名或密码为空或格式不正确", nil)
		os.Exit(0)
	}
	if strings.Contains(conf.IP, "//") || conf.IP == "" {
		StdOutUnit.Warn.String("", "数据库IP为空或格式不正确，将使用默认值", nil)
		conf.IP = "localhost"
	}
	_, err = strconv.Atoi(conf.Port)
	if err != nil {
		StdOutUnit.Warn.String("", "数据库端口为空或格式不正确，将使用默认值", nil)
		conf.Port = "3306"
	}
	if strings.Contains(conf.DBName, "//") || conf.DBName == "" {
		StdOutUnit.Warn.String("", "数据簿名称为空或格式不正确，将使用默认值", nil)
	...
	if err != nil {
		StdOutUnit.Assert.String("", "数据库模块初始化失败", err)
		os.Exit(0)
	}
	err = Maria.Ping()
	if err != nil {
		StdOutUnit.Assert.String("", "数据库连接失败", err)
		os.Exit(0)
	}
	StdOutUnit.Verbose.String("", "SQL配置成功")
}
