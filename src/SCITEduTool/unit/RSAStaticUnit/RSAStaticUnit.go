...
func SetPrivateKey(keyConf PrivateKey) {
	if keyConf.Content == "" || strings.Contains(keyConf.Content, "//") {
		StdOutUnit.Assert("", "私钥数据为空或格式错误", nil)
		os.Exit(0)
	}
	key, err := base64.StdEncoding.DecodeString(keyConf.Content)
	if err != nil {
		StdOutUnit.Assert("", "私钥数据解密失败", err)
		os.Exit(0)
	}
	block, _ := pem.Decode(key)
	privateKey, err = x509.ParsePKCS1PrivateKey(block.Bytes)
	if err != nil {
		StdOutUnit.Assert("", "私钥解析失败", err)
		os.Exit(0)
	}
	StdOutUnit.Verbose("", "RSA配置成功")
}
